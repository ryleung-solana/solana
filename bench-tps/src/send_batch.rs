...
pub fn get_latest_blockhash<T: GenericClient + ?Sized>(client: &T) -> Hash {
    loop {
...
/// or full
pub fn fund_keys<T: 'static + GenericClient + Send + Sync + ?Sized>(
    client: Arc<T>,
    source: &Keypair,
    dests: &[Keypair],
    total: u64,
...
pub fn generate_durable_nonce_accounts<T: 'static + GenericClient + Send + Sync + ?Sized>(
    client: Arc<T>,
...
pub fn withdraw_durable_nonce_accounts<T: 'static + GenericClient + Send + Sync + ?Sized>(
    client: Arc<T>,
    authority_keypairs: &[Keypair],
...
fn verify_funding_transfer<T: GenericClient + ?Sized>(
    client: &Arc<T>,
    tx: &Transaction,
    amount: u64,
) -> bool {
...
trait SendBatchTransactions<'a, T: Sliceable + Send + Sync> {
    ...
    where
        C: 'static + GenericClient + Send + Sync + ?Sized,
        F: Fn(usize, usize);
    fn sign(&mut self, blockhash: Hash);
    fn send<C: GenericClient + ?Sized>(&self, client: &Arc<C>);
    fn verify<C: 'static + GenericClient + Send + Sync + ?Sized>(
        &mut self,
        client: &Arc<C>,
        to_lamports: u64,
    );
...
trait FundingTransactions<'a>: SendBatchTransactions<'a, FundingSigners<'a>> {
    fn fund<T: 'static + GenericClient + Send + Sync + ?Sized>(
        &mut self,
        client: &Arc<T>,
...
trait NonceTransactions<'a>: SendBatchTransactions<'a, NonceCreateSigners<'a>> {
    fn create_accounts<T: 'static + GenericClient + Send + Sync + ?Sized>(
        &mut self,
        client: &Arc<T>,
...
trait NonceWithdrawTransactions<'a>: SendBatchTransactions<'a, NonceWithdrawSigners<'a>> {
    fn withdraw_accounts<T: 'static + GenericClient + Send + Sync + ?Sized>(
        &mut self,
        client: &Arc<T>,
