...
fn run_transactions_dos(
    ...
    let executor = TransactionExecutor::new(entrypoint_addr, true);

    let mut accounts_created = false;
    let mut tested_size = false;
                ...
                let txs: Vec<_> = (0..chunk_size)
                    ...
                    .map(|x| {
                        let message = make_dos_message(
                            ...
                            &account_metas[(x%account_groups)*account_group_size.. (x%account_groups)*account_group_size + account_group_size],
                        );
                        let signers: Vec<&Keypair> = vec![keypair];
                        let tx = Transaction::new(&signers, message, blockhash);
                        if !tested_size {
                            assert!(bincode::serialized_size(&tx).unwrap() < 1200);
                            tested_size = true;
                        }
                        tx
                    })
